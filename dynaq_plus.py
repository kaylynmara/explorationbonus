...
def dyna_q_control(seed):
    """exploration bonus implemented in model updates"""
    random.seed(seed)
    np.random.seed(seed)

    Q = {}
    model = {}
    tau = {} # time since each (s,a) pair was last tried
    time = 0 # counter
    episode_rewards = []
    episode_steps = []
    ...
    for ep in range(episodes):
        ...
        while True:
            ...
            for _ in range(planning_steps):
                ...
                bonus = kappa * np.sqrt(tau.get((s, a), 0))  # exploration bonus
                Q[s][a] += alpha * ((r + bonus) + gamma * max(Q[s_prime].values()) - Q[s][a])
