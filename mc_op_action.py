...
def bonus_behavior_policy(state, Q, returns, eps_behavior):
    if state not in Q:
...
def generate_episode(env, Q, returns, eps_behavior):
    env.reset()
    episode = []
    done = False
    total_reward = 0
    ...
    while not done:
        ...
        action = bonus_behavior_policy(state, Q, returns, eps_behavior)
...
def op_monte_carlo_control(seed):
    ...
    for episode in range(episodes):
        ...
        episode_data, total_reward, steps = generate_episode(env, Q, returns={}, eps_behavior=eps_behavior)
